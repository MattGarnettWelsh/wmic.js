...
function func(options) {
    ...
    var nodes = Array.isArray(_options.node)
        ? _options.node.join(",")
        : _options.node;
    var switches = [
        ...
        "/PRIVILEGES:" +
            (_options.privileges &&
            _options.privileges.toUpperCase() === "DISABLE"
                ? "DISABLE"
                : "ENABLE"),
        "/TRACE:OFF",
        "/INTERACTIVE:OFF",
    ...
    return {
        alias: function alias(friendlyName) {
            return createQuery.apply(
                undefined,
                [_options.exec, _options.binary].concat(
                    (0, _toConsumableArray3.default)(switches),
                    [friendlyName]
                )
            );
        },
        ...
        context: function context() {
            var command = [_options.binary]
                .concat((0, _toConsumableArray3.default)(switches), ["context"])
                .join(" ");
